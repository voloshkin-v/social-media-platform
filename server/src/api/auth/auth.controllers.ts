...
const createSendTokens = async (user: IUser, statusCode: number, res: Response, next: NextFunction) => {
    try {
...
export const refresh = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        const userData = jwt.verify(refreshToken, process.env.REFRESH_TOKEN_SECRET!) as RefreshToken;

        const tokenFromDb = await tokenService.findToken(refreshToken);

        if (!tokenFromDb) {
            return next(new AppError('This token does not belong to you. Access denied', 401));
        }
